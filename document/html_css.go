...
func (index *htmlCSSRuleIndex) selectorsForElement(el HTMLSegmentType) []htmlCSSSelectorRef {
	if index == nil || len(index.buckets) == 0 {
		return nil
	}
	var refs []htmlCSSSelectorRef
	appendBucket := func(key string) {
		if bucket := index.buckets[key]; len(bucket) > 0 {
	...
	for _, className := range htmlClassNames(el.Attr["class"]) {
		appendBucket("class:" + className)
	}
	if el.Str != "" {
		appendBucket("tag:" + el.Str)
	}
	appendBucket("*")
	return refs
}
...
func (index *htmlCSSRuleIndex) selectorsForElementMeta(meta htmlElementMetadata) []htmlCSSSelectorRef {
	...
	if meta.id != "" {
		appendBucket("id:" + meta.id)
	}
	for _, className := range meta.classes {
		appendBucket("class:" + className)
	}
	if meta.tag != "" {
		appendBucket("tag:" + meta.tag)
	}
	appendBucket("*")
	return refs
}
