...
const maxGridTickmarkCount = 100000

func extent(min, div float64, count int) (tm []float64, precision int, ok bool) {
	if count < 0 || count > maxGridTickmarkCount || !finiteNumbers(min, div) {
		return nil, 0, false
	}
...
func (g *GridType) TickmarksExtentX(min, div float64, count int) {
	var ok bool
	g.xTicks, g.xPrecision, ok = extent(min, div, count)
	if !ok {
		g.xTicks = nil
		g.xPrecision = 0
		return
...
func (g *GridType) TickmarksExtentY(min, div float64, count int) {
	var ok bool
	g.yTicks, g.yPrecision, ok = extent(min, div, count)
	if !ok {
		g.yTicks = nil
		g.yPrecision = 0
		return
