...
func TestSecurityCompareHelpers(t *testing.T) {
	if err := ComparePDFs(bytes.NewReader([]byte("same")), bytes.NewReader([]byte("same")), false); err != nil {
		t.Fatalf("ComparePDFs() error = %v", err)
	}
	if err := CompareBytes([]byte("a"), []byte("ab"), false); err == nil {
...
func TestSecurityTemplateTypedNilChildReturnsError(t *testing.T) {
	tpl := &FpdfTpl{
		bytes:     [][]byte{nil, []byte("q")},
		page:      1,
		templates: []Template{(*FpdfTpl)(nil)},
	}
	defer func() {
		if r := recover(); r != nil {
...
func TestSecurityHugeType1SegmentRejected(t *testing.T) {
	var input bytes.Buffer
	input.WriteByte(128)
	input.WriteByte(1)
	if err := binary.Write(&input, binary.LittleEndian, uint32(maxImageSourceBytes+1)); err != nil {
