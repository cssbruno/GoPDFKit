...
func ExampleTtfParse() {
	ttf, err := gopdfkit.TtfParse(example.FontDir() + "/calligra.ttf")
	if err == nil {
...
func TestTtfParseTruncatedFontReturnsReadError(t *testing.T) {
	dir := t.TempDir()
	...
	_, err := gopdfkit.TtfParse(fontPath)
	if err == nil {
		t.Fatal("expected truncated font error")
	}
	if !strings.Contains(err.Error(), "EOF") {
		t.Fatalf("TtfParse error = %q, want EOF", err.Error())
	}
}

func TestOpenTypeParseCFFFont(t *testing.T) {
	dir := t.TempDir()
	...
	otf, err := gopdfkit.OpenTypeParse(fontPath)
	if err != nil {
		t.Fatalf("OpenTypeParse: %s", err)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "not a TrueType font") {
		t.Fatalf("TtfParse error = %v, want not a TrueType font", err)
	}
}

func TestMakeFontOpenTypeCFF(t *testing.T) {
	dir := t.TempDir()
	fontPath := filepath.Join(dir, "minimal.otf")
	cffData := []byte{1, 0, 4, 4, 0, 1, 1, 1}
	if err := os.WriteFile(fontPath, minimalOpenTypeCFF(cffData), 0644); err != nil {
	...
	err := gopdfkit.MakeFont(fontPath, example.FontFile("cp1252.map"), dir, io.Discard, true)
	if err != nil {
		t.Fatalf("MakeFont: %s", err)
	}
...
func ExampleFpdf_GetStringWidth() {
	pdf := gopdfkit.New("", "", "", example.FontDir())
