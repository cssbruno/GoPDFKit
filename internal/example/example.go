...
// setRoot assigns the relative path to the gopdfkitDir directory based on current working
// directory
func setRoot() {
	wdStr, err := os.Getwd()
	if err == nil {
		...
		for j := len(list) - 1; j >= 0 && list[j] != "gopdfkit"; j-- {
...
// ImageFile returns a qualified filename in which the path to the static image
...
// FontFile returns a qualified filename in which the path to the static font
// directory is prepended to the specified filename.
func FontFile(fileStr string) string {
	return filepath.Join(FontDir(), fileStr)
}

// TextFile returns a qualified filename in which the path to the static text
// directory is prepended to the specified filename.
