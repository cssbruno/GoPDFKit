...
func htmlBorderFromAttrs(attrs map[string]string, pdf *Fpdf, relative float64) htmlBorderStyle {
...
func htmlBorderFromDeclarations(decl map[string]string, pdf *Fpdf, relative float64) htmlBorderStyle {
	border := htmlBorderStyle{}
...
func htmlBorderSideFromDeclarations(decl map[string]string, side string, fallback htmlBorderStyle, pdf *Fpdf, relative float64) (htmlBorderSideStyle, bool) {
	prefix := "border-" + side
...
func parseHTMLBorderWidth(value string, pdf *Fpdf, relative float64) (float64, bool) {
	for _, token := range strings.Fields(strings.ToLower(strings.TrimSpace(value))) {
		switch token {
		case "thin":
			return 0.5, true
...
func htmlApplyBorderStyle(pdf *Fpdf, border htmlBorderStyle, fallbackR, fallbackG, fallbackB int, fallbackWidth float64) {
...
func htmlDrawBorderedRect(pdf *Fpdf, x, y, wd, ht float64, border htmlBorderStyle, fill bool, fallbackR, fallbackG, fallbackB int, fallbackWidth float64) {
	if fill {
...
func htmlDrawBorderSide(pdf *Fpdf, side htmlBorderSideStyle, fallbackR, fallbackG, fallbackB int, fallbackWidth, x1, y1, x2, y2 float64) {
	if !side.enabled {
		return
	}
...
func applyHTMLAttrs(st *htmlTextStyle, attrs map[string]string, baseFontSize, baseLineHeight float64, pdf *Fpdf) {
	if attrs == nil {
		return
	}
...
func applyHTMLStyleDeclarations(st *htmlTextStyle, style map[string]string, baseFontSize, baseLineHeight float64, pdf *Fpdf) {
...
func parseHTMLLineHeight(value string, base float64, pdf *Fpdf) (float64, bool) {
	value = strings.TrimSpace(strings.ToLower(value))
