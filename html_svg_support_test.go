...
func TestHTMLTokenizeAttributesAndEntities(t *testing.T) {
	segments := gopdfkit.HTMLTokenize(`<a href="https://example.test?q=a b" title='A &amp; B'>A&nbsp;B</a><br/>`)
	if len(segments) != 5 {
...
func TestHTMLTokenizeQuotedTagEndAndComments(t *testing.T) {
	segments := gopdfkit.HTMLTokenize(`<p title="a > b">keep</p><!-- skip <b>comment</b> --><span data-x='c > d'>tail</span>`)
	if len(segments) != 6 {
...
func TestHTMLWriteTableRepeatsHeaderRowsAcrossPages(t *testing.T) {
	pdf := gopdfkit.NewCustom(&gopdfkit.InitType{
		UnitStr: "mm",
		Size:    gopdfkit.Size{Wd: 80, Ht: 70},
	})
	pdf.SetCompression(false)
...
func TestSVGParseStylesAndText(t *testing.T) {
	svg, err := gopdfkit.SVGParse([]byte(`<svg width="100" height="50">
...
func TestSVGParseCurrentColor(t *testing.T) {
	svg, err := gopdfkit.SVGParse([]byte(`<svg width="30" height="20">
...
func TestSVGParseEmbeddedImage(t *testing.T) {
	const pngData = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJ" +
		"AAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="
	svg, err := gopdfkit.SVGParse([]byte(`<svg width="40" height="30">
...
func TestSVGWriteStyledContent(t *testing.T) {
	svg, err := gopdfkit.SVGParse([]byte(`<svg width="100" height="40">
...
func TestSVGWriteEvenOddFillRule(t *testing.T) {
	svg, err := gopdfkit.SVGParse([]byte(`<svg width="20" height="20">
...
func TestSVGParseClipPathAndLinearGradient(t *testing.T) {
	svg, err := gopdfkit.SVGParse([]byte(`<svg width="40" height="40">
		<style>
...
func TestSVGWriteClipPathAndGradients(t *testing.T) {
	svg, err := gopdfkit.SVGParse([]byte(`<svg width="80" height="40">
		<defs>
...
func TestSVGParseViewBoxAndTransforms(t *testing.T) {
	svg, err := gopdfkit.SVGParse([]byte(`<svg width="100" height="50" viewBox="-10 -20 20 10">
