...
func SignPDFBytes(input []byte, options SignOptions) ([]byte, error) {
	if len(input) == 0 {
...
func SignPDFFile(inputPath, outputPath string, options SignOptions) error {
	...
	signed, err := SignPDFBytes(input, options)
	if err != nil {
		return err
	}
...
func signPDFContext(ctx pdfContext, options SignOptions) ([]byte, error) {
...
func buildIncrement(ctx pdfContext, options SignOptions, byteRangePlaceholder, contentsPlaceholder string) ([]byte, error) {
...
func signatureDictionary(options SignOptions, byteRangePlaceholder, contentsPlaceholder string, signingTime time.Time) string {
