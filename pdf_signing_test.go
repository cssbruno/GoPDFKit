...
func TestSignPDFBytesRequiresSigner(t *testing.T) {
	cert, _ := testSigner(t)
	_, err := SignPDFBytes(testPDFBytes(t), SignOptions{Certificate: cert})
	if !errors.Is(err, ErrMissingSigner) {
		t.Fatalf("SignPDFBytes() error = %v, want ErrMissingSigner", err)
	}
}

func TestSignPDFBytesRejectsSHA1Digest(t *testing.T) {
	cert, signer := testSigner(t)
	_, err := SignPDFBytes(testPDFBytes(t), SignOptions{
		Signer:          signer,
		Certificate:     cert,
		DigestAlgorithm: crypto.SHA1,
	})
	if err == nil {
		t.Fatal("SignPDFBytes() accepted SHA-1 digest")
	}
	if !strings.Contains(err.Error(), "insecure digest") {
		t.Fatalf("SignPDFBytes() error = %v, want insecure digest error", err)
	}
}

func TestSignPDFBytesAndVerifyPDFBytes(t *testing.T) {
	cert, signer := testSigner(t)
	truststore := x509.NewCertPool()
	truststore.AddCert(cert)

	signedPDF, err := SignPDFBytes(testPDFBytes(t), SignOptions{
		Signer:          signer,
		Certificate:     cert,
	...
	if err != nil {
		t.Fatalf("SignPDFBytes() error = %v", err)
	}
	...
	signature, err := VerifyPDFBytes(signedPDF, truststore)
	if err != nil {
...
func TestVerifyPDFBytesRejectsSignedRangeTampering(t *testing.T) {
	cert, signer := testSigner(t)
	truststore := x509.NewCertPool()
	truststore.AddCert(cert)
	...
	if _, err := VerifyPDFBytes(tamperedPDF, truststore); err == nil {
		t.Fatal("VerifyPDFBytes() accepted tampered signed content")
	}
}

func TestSignPDFFileWritesOwnerOnlyOutput(t *testing.T) {
	cert, signer := testSigner(t)
	dir := t.TempDir()
	...
	err := SignPDFFile(inputPath, outputPath, SignOptions{
		Signer:          signer,
		Certificate:     cert,
		DigestAlgorithm: crypto.SHA256,
		SigningTime:     time.Now().UTC(),
	...
	if err != nil {
		t.Fatalf("SignPDFFile() error = %v", err)
	}
...
func TestVerifyPDFBytesRejectsUnsignedTrailingData(t *testing.T) {
	cert, signer := testSigner(t)
	truststore := x509.NewCertPool()
	truststore.AddCert(cert)
	...
	_, err = VerifyPDFBytes(tamperedPDF, truststore)
	if err == nil {
		t.Fatal("VerifyPDFBytes() accepted unsigned trailing data")
	}
	if !strings.Contains(err.Error(), "does not cover full PDF") {
		t.Fatalf("VerifyPDFBytes() error = %v, want full PDF coverage error", err)
	}
}
...
func TestSignPDFBytesRejectsHugeSignatureSize(t *testing.T) {
	cert, signer := testSigner(t)
	_, err := SignPDFBytes(testPDFBytes(t), SignOptions{
		Signer:          signer,
		Certificate:     cert,
	...
	if err == nil {
		t.Fatal("SignPDFBytes() accepted huge SignatureSize")
	}
}
...
func TestSignPDFBytesUsesIncrementPlaceholders(t *testing.T) {
	cert, signer := testSigner(t)
	truststore := x509.NewCertPool()
	truststore.AddCert(cert)
	...
	signedPDF, err := SignPDFBytes(input, SignOptions{
		Signer:          signer,
		Certificate:     cert,
		DigestAlgorithm: crypto.SHA256,
		SigningTime:     time.Now().UTC(),
	...
	if _, err := VerifyPDFBytes(signedPDF, truststore); err != nil {
		t.Fatalf("VerifyPDFBytes() error = %v", err)
	}
}
